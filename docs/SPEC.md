...
- `igalhub/project-template` scaffold could not be pulled into this repo (private/inaccessible without an authorized GitHub connector in this session) — root scaffold (`.github/workflows`, pre-commit config, CLAUDE.md workflow) was hand-built from the documented convention instead. Worth diffing against the real template once GitHub access is available.
